...
const LatestNews: React.FC<ILatestNewsProps> = (props: ILatestNewsProps) => {
    ...
    const [showCommentPart, setShowCommentPart] = React.useState<{ [key: string]: boolean }>({});
    ...
    const handleToggleCommentPart = (newsNews: string) => {
        setShowCommentPart(prevState => ({
            ...prevState,
            [newsNews]: !prevState[newsNews],
        }));
    };

    const handleSubmitComment = async (newsNews: string) => {
        try {
            const commentText = (document.getElementById(`subject-${newsNews}`) as HTMLTextAreaElement).value;
            const commentService = new CommentService();
            await commentService.postComment(commentText, newsNews);
                        ...
                        <div className={styles.cards_container} >
                            ...
                            {newsData.map((item, index) => (
                                <div className={styles.card} key={index}>
                                    ...
                                    {showCommentPart[item.News] ? (
                                        <div className={styles.CommentPart}>
                                            <textarea id={`subject-${item.News}`} name="subject" placeholder="tapez votre commentaire.."  style={{ height: '100px', fontSize: '11px', background:'#dff9f1' }}></textarea>
                                            <button className={styles.CommentPart_btn} onClick={() => handleSubmitComment(item.News)}>Submit</button>
                                        </div>
                                    ) : null}
                                </div>
                            ))}
